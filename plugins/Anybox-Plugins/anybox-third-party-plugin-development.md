...
当前代码库里仍沿用部分 `Fanfande` 命名，例如环境变量 `FANFANDE_PLUGIN_INSTALL_DIR`。本文中的 Anybox 插件系统指的就是当前桌面端 Agent 使用的这套插件运行时。
...
`.fanfande-plugin/plugin.json` 是插件清单。它是严格 JSON，未知顶层字段会被拒绝。
...
```powershell
$env:FANFANDE_PLUGIN_INSTALL_DIR = "C:\Projects\anybox-plugin-examples"
```

但要注意：当前实现把 `FANFANDE_PLUGIN_INSTALL_DIR` 视为受管理安装目录。用户在 UI 里卸载插件时，可能会删除该目录下对应插件包。
...
```text
hello-anybox/0.1.0/.fanfande-plugin/plugin.json
```
...
```powershell
$env:FANFANDE_PLUGIN_INSTALL_DIR = "C:\Projects\anybox-plugin-examples"
$env:FANFANDE_PLUGIN_REGISTRY_INDEX_URL = "off"
```

然后在 Anybox/Fanfande Agent 源码仓库里运行 catalog 检查：
...
- `FANFANDE_PLUGIN_INSTALL_DIR` 是否指向插件集合根目录。
- 目录是否是 `<plugin-id>/<version>/.fanfande-plugin/plugin.json`。
...
如果你把 Git 仓库根目录直接设置为 `FANFANDE_PLUGIN_INSTALL_DIR`，UI 卸载可能删除其中的插件包。长期开发建议使用 `dev-install` 作为运行目录，把源码目录和安装目录分开。
