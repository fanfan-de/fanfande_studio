...
try {
  ...
  for (const pluginName of pluginDirs) {
    ...
    const meta = {
      ...
      package: {
        type: "zip",
        url: `https://raw.githubusercontent.com/fanfan-de/Anybox-Plugins/main/${pluginID}/${zipName}`,
...
async function updateIndex() {
  const dirs = (await readdir(repoRoot, { withFileTypes: true }))
    ...
    .sort((left, right) => left.localeCompare(right))
  const urls = dirs.map((name) => `https://raw.githubusercontent.com/fanfan-de/Anybox-Plugins/main/${name}`)
