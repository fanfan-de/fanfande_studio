...
async function resolveManagedAgentRuntimeDir() {
  for (const candidate of resolveRuntimeCandidates()) {
...
export async function ensureManagedAgentRunning() {
  ...
  const runtimeDir = await resolveManagedAgentRuntimeDir()
  if (!runtimeDir) {
  ...
  const bunBinary = path.join(runtimeDir, BUNDLED_BUN_BINARY)
  const entrypoint = path.join(runtimeDir, BUNDLED_AGENT_ENTRYPOINT)
  const dataDir = path.join(app.getPath("userData"), "agent")

  await fsp.mkdir(dataDir, { recursive: true })

  const child = spawn(bunBinary, [entrypoint], {
    cwd: dataDir,
  ...
  try {
    ...
    log(`managed agent ready at ${baseURL}`)
    return baseURL
  } catch (error) {
    managedAgent = undefined
    try {
    ...
    logError("managed agent failed to start", error)
    throw error
  }
}
