...
describe("WorkspaceFilesPanel", () => {
  ...
  it("keeps file reader text on readable panel colors", () => {
    ...
    expect(styles).toMatch(
      /\.right-sidebar-view-host\.is-preview,\s*\.right-sidebar-view-host\.is-files,\s*\.right-sidebar-view-host\.is-changes,\s*\.right-sidebar-view-host\.is-terminal\s*\{[^}]*scrollbar-gutter:\s*auto;[^}]*padding-right:\s*0;/s,
    )
    expect(styles).toMatch(
