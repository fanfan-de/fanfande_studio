...
describe("App", () => {
  ...
  it("scopes provider scrolling to the column layout", () => {
    ...
    expect(styles).toMatch(/\.settings-page-main\.is-services\.prompt-presets-page-main\s+\.settings-prompt-assignment-row\s*\{[^}]*position:\s*relative;[^}]*padding:\s*10px 10px 11px 13px;[^}]*border-left:\s*0;[^}]*border-radius:\s*8px;[^}]*background:\s*rgba\(255,\s*255,\s*255,\s*0\.32\);[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\);[^}]*gap:\s*8px;/s)
    expect(styles).toMatch(/\.settings-page-main\.is-services\.prompt-presets-page-main\s+\.settings-prompt-assignment-row::before\s*\{[^}]*content:\s*"";[^}]*width:\s*2px;[^}]*background:\s*rgba\(120,\s*130,\s*140,\s*0\.28\);/s)
    expect(styles).toMatch(/\.settings-page-main\.is-services\.prompt-presets-page-main\s+\.settings-prompt-assignment-row\.is-open::before\s*\{[^}]*background:\s*rgba\(143,\s*99,\s*233,\s*0\.66\);/s)
