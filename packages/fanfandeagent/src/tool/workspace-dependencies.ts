...
const FANFANDE_NODE_BINARY_ENV = "FanFande_NODE_BINARY"
const FANFANDE_NODE_RUN_AS_NODE_ENV = "FanFande_NODE_RUN_AS_NODE"
...
function resolveNodeExecutable() {
  const configured = process.env[FANFANDE_NODE_BINARY_ENV]?.trim()
...
export function loadWorkspaceDependencies(): WorkspaceDependenciesData {
  const dependenciesRoot = process.env[WORKSPACE_DEPENDENCIES_DIR_ENV]?.trim() ?? ""
  const manifest = readManifest(dependenciesRoot)
  const bundleVersion =
    process.env[WORKSPACE_DEPENDENCIES_VERSION_ENV]?.trim() ||
  ...
  const nodeEnv: Record<string, string> =
    process.env[FANFANDE_NODE_RUN_AS_NODE_ENV] === "1"
  ...
  if (!nodeExecutable) {
    missing.push(FANFANDE_NODE_BINARY_ENV)
