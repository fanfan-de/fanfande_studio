...
const CONNECTOR_REGISTRY_FILES_ENV = "FANFANDE_CONNECTOR_REGISTRY_FILES"
...
export const ConnectorDefinition = z
  .object({
    ...
    publisher: z.string().min(1).default("Fanfande"),
...
function registryFilePaths() {
  return (process.env[CONNECTOR_REGISTRY_FILES_ENV] ?? "")
    .split(delimiter)
