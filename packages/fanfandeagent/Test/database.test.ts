...
test("toSQLiteValue converts objects correctly", () => {
  ...
  const result = (Sqlite as any).toSQLiteValue(input)
  
  expect(result.id).toBe("123")
  expect(result.name).toBe("Alice")
  expect(result.age).toBe(30)
  expect(result.tags).toBe('["dev","test"]') // JSON 序列化
  expect(result.createdAt).toBe(new Date("2025-01-01T00:00:00Z").getTime()) // 时间戳
  expect(result.active).toBe(true) // 布尔值保持不变
  expect(result.nullField).toBeNull()
  expect(result.undefinedField).toBeNull()
})
