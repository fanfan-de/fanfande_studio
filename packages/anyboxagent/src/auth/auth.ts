...
export const OAuthClientRegistrationRecord = z
  .object({
  ...
  .meta({ ref: "OAuthClientRegistrationRecord" })
export type OAuthClientRegistrationRecord = z.infer<typeof OAuthClientRegistrationRecord>
...
export const ProviderAuthRecord = z
  .object({
    ...
    oauthClientRegistrations: z.record(z.string(), OAuthClientRegistrationRecord).default({}),
...
export async function getOAuthClientRegistration(
  providerID: string,
  key: string,
): Promise<OAuthClientRegistrationRecord | undefined> {
  const record = await getProviderRecord(providerID)
  return record?.oauthClientRegistrations[key]
}
...
export async function setOAuthClientRegistration(
  providerID: string,
  key: string,
  ...
  const nextRegistration = {
    ...registration,
    createdAt: registration.createdAt ?? timestamp,
    updatedAt: timestamp,
  }
  ...
  store.providers[providerID] = {
    ...current,
    oauthClientRegistrations: {
      ...(current.oauthClientRegistrations ?? {}),
      [key]: nextRegistration,
    },
    updatedAt: timestamp,
  }
...
export async function removeProviderCredential(providerID: string, method: string) {
  ...
  if (
    ...
    Object.keys(current.oauthClientRegistrations ?? {}).length === 0 &&
    !current.lastError
  ) {
...
export async function removeOAuthClientRegistration(providerID: string, key: string) {
  const store = await getStore()
  const current = store.providers[providerID]
  if (!current?.oauthClientRegistrations[key]) return false

  const nextRegistrations = { ...current.oauthClientRegistrations }
  delete nextRegistrations[key]

  if (Object.keys(current.credentials).length === 0 && Object.keys(nextRegistrations).length === 0 && !current.lastError) {
    delete store.providers[providerID]
  } else {
    store.providers[providerID] = {
      ...current,
      oauthClientRegistrations: nextRegistrations,
      updatedAt: Date.now(),
    }
  }
